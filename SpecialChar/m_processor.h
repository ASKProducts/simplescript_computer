...
#ifndef SpecialChar_m_processor_h
#define SpecialChar_m_processor_h
#include "Data.h"
#include "Autoreleasepool.h"
#include "Dictionary.h"
bool loop_recording = NO;
bool file_recording = NO;
bool if_mode[10] = {YES,YES,YES,YES,YES,YES,YES,YES,YES,YES};
ubyte if_depth = 0;
String *loop;
String *file;
String filename;
...
int a;
String processCode(String code){
    if(!if_mode[if_depth]){StrcmpIf(code, ==, "endif")if_mode[if_depth--]=!if_mode[if_depth];return "Success";}
    else StrcmpIf(code, ==, "endif"){if_depth--;return "Success";}
    if(loop_recording){
    ...
    StrcmpIf(code, ==, "save"){file_recording = NO;return "Success";}
    ...
    StrcmpIf(c.words[0], ==, "if"){
        task = "if";
        if_depth++;
        extra = c.words[2];
        Number first;
        Number second;
        if(c.words[1][0] >= '-' && c.words[1][0] <= '9') first=atof(c.words[1]);
        else first = getFourByteForKey(_fbmd, c.words[1]);
        
        if(c.words[3][0] >= '-' && c.words[3][0] <= '9') second=atof(c.words[3]);
        else second = getFourByteForKey(_fbmd, c.words[3]);
        
        if(first == -1){if_mode[if_depth]=NO;return "Unknown Variable, not performing 'if' call";}
        else first = getN(first);
        
        if(second == -1){if_mode[if_depth]=NO;return "Unknown Variable, not performing 'if' call";}
        else second = getN(second);
        
        
        StrcmpIf(extra, ==, "=")if_mode[if_depth]=(first==second);
        StrcmpIf(extra, ==, "!=")if_mode[if_depth]=(first!=second);
